...
const ResolvedPage = ({ posts }: { posts:Record[] }) => {
  const [resolvedPosts, setRedflagPosts] = useState(posts);
  ...
  // // Filter posts that are resolved
  // const resolvedPosts = redflagPosts.filter((post) => post.status === "Resolved");
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-semibold text-white mb-4">Resolved Posts</h2>

      {/* Display the resolved posts */}
      {resolvedPosts.length === 0 ? (
        <p className="text-gray-400">No resolved posts found.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {resolvedPosts.map((post) => (
            ...
            >
              ...
              <p className="text-sm text-green-400 mt-2">Status: {post.status}</p>
